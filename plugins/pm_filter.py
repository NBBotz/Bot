...
async def give_filter(client, message):
    ...
    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(" ᴊᴏɪɴ ᴏᴜʀ ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ  ", url=f"https://t.me/All_Movies_Studio_Movie_Search")]])
)
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>ʜᴇʏ {user} 😍 ,\n\nBhai Idher Pe Movies Nahi Milaga. Hamara Group Join Karlo Uska Bad Uder Jaka Search Karo.... <a href=https://t.me/All_Movies_Studio_Movie_Search>Movies</a> ᴏʀ Nicha Wala Button Pe Click Karke Ap Humra Movies Group Pe Join Ho Sakta Ho 👇</b>",   
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton(f'Qᴜᴀʟɪᴛɪᴇs', callback_data=f"qualities#{key}"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/All_Movies_Studio_Request")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_text(f"<b><i> 𝖲𝖾𝖺𝗋𝖼𝗁𝗂𝗇𝗀 For '{search}' 🔎</i></b>")
            search = search.lower()
            find = search.split(" ")
            search = ""
